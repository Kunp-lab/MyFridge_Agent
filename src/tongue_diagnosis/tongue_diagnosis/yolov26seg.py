...
class YOLO26SegConfig:
    ...
    strides: np.ndarray = field(
        default_factory=lambda: np.array([32, 16, 8], dtype=np.int32)
    )
...
class YOLO26Seg:
    ...
    def __init__(self, config: YOLO26SegConfig):
        ...
        logger.info(
            f"\033[1;31m[Seg] Load Model time = {1000 * (time.time() - t0):.2f} ms\033[0m"
        )
    ...
    def set_scheduling_params(
        self, priority: Optional[int] = None, bpu_cores: Optional[list] = None
    ) -> None:
        """
    ...
    def pre_process(
        ...
        if image_format == "BGR":
            resize_img = pre_utils.resized_image(
                img, self.input_w, self.input_h, resize_type
            )
        ...
        logger.info(
            f"\033[1;31m[Seg] Pre-process time = {1000 * (time.time() - t0):.2f} ms\033[0m"
        )
    ...
    def forward(
        ...
        logger.info(
            f"\033[1;31m[Seg] Forward time = {1000 * (time.time() - t0):.2f} ms\033[0m"
        )
        return outputs
    ...
    def post_process(
        ...
        for i, stride in enumerate(self.cfg.strides):
            ...
            layer_pred = decode_seg_layer(
                box_feat, cls_feat, mc_feat, stride, score_thres, self.cfg.classes_num
            )
        ...
        masks = process_mask(
            proto_tensor, mask_coefs, xyxy, (ori_img_h, ori_img_w), upsample=True
        )

        xyxy = post_utils.scale_coords_back(
            xyxy, ori_img_w, ori_img_h, self.input_w, self.input_h, self.cfg.resize_type
        )

        logger.info(
            f"\033[1;31m[Seg] Post Process time = {1000 * (time.time() - t0):.2f} ms\033[0m"
        )
